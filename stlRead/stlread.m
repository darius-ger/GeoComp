...
if( isbinary(M) ) % This may not be a reliable test
    fprintf('\n Reading binary .stl file %s.\n\n',file);
    [F,V,N] = stlbinary(M);
else
    fprintf('\n Reading ASCII .stl file %s.\n\n',file);
    [F,V,N] = stlascii(file);
end
...
% %
% fprintf('Align normals to point outwards.\n');
% F = unifyMeshNormals(F,V,'alignTo','out');

%% Calculate normals
%
if isempty(N) && ~isequal(size(N), size(F))
   N = normals(F,V);
end
